...
function Autocomplete:_GetCursorPosition() : UDim2
    ...
    local CursorXOffset = TextService:GetTextSize(Text:sub(1, CursorPosition), FontSize, Font, Vector2.new(10000, 10000)).X
...
function Autocomplete:_Hide()
    ...
    for i, v in ipairs(self.AutocompleteFrame:GetChildren()) do
...
function Autocomplete:_Show()
    ...
    for _, v in ipairs(MatchingKeywords) do
        local Template = self.AutocompleteButton:Clone()
        Template.Text = v.Keyword
        Template.Visible = true
        Template.Parent = self.AutocompleteFrame

        Template.InputEnded:Connect(function(Input)
